...
void touch_init(void)
{
  ...
  PINSEL_ConfigPin(&touch_init_cfg);
    ...
    // NVIC_EnableIRQ(EINT3_IRQn); pas géré en interruption mais par scrutation
...
void touch_debug(void)
{
    ...
    // LCD_write_english_string(0,290,"          ",White,Blue);
    // LCD_write_english_string(0,300,"touch down",White,Blue);
    ...
    while( TP_DOWN() )
    {
        ...
        //display
        {
            ...
            LCD_write_english_string(0,290,x_str,Cyan,Blue);
            LCD_write_english_string(120,290,y_str,Cyan,Blue);
            //_printf("\rtouch down!");
            // _printf("X:%04d Y:%04d         ",touch_x,touch_y);
        }
    }// Èç¹ûÒ»Ö±°´ÏÂ,¾ÍÒ»Ö±¶ÁÈ¡²¢ÏÔÊ¾Ô­Ê¼×ø±ê
...
void touch_read(void)
{
    ...
    for(i=0; i<10; i++)
        ...
        //calcul d'une valeur moyenne sur les 8 valeurs les plus crédibles
        {
            uint32_t min_x = 0xFFFF,min_y = 0xFFFF;
            uint32_t max_x = 0,max_y = 0;
            uint32_t total_x = 0;
